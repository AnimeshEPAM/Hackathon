...
public class FirefoxLauncher implements DriverLauncher {
    ...
    private DriverDTO initializeTheFirefoxDriverForSauceLab() {
        ...
        try {
            driver = new RemoteWebDriver(new URL(UiConstants.SAUCE_HUB_URL), firefoxCapabilities);
    ...
    private MutableCapabilities getFirefoxCapabilities() {
        DesiredCapabilities firefoxCapabilities = (DesiredCapabilities) getFirefoxCapabilities();
    ...
    private MutableCapabilities getSauceCapabilities(String scenarioName) {
        ...
        sauceCapabilities.setCapability("username", UiConstants.USER_NAME);
        sauceCapabilities.setCapability("accessKey", UiConstants.ACCESS_KEY);
        sauceCapabilities.setCapability("seleniumVersion", "3.141.59");
        sauceCapabilities.setCapability("name", scenarioName);
        sauceCapabilities.setCapability("build", UiConstants.BUILD_NAME);
